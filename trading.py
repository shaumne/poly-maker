...
try:
    ...
    DRY_RUN_ENABLED = Config.is_dry_run()
except ImportError:
...
async def perform_trade(market):
    ...
    async with market_locks[market]:
        try:
            ...
            # Filter which sides to trade based on configuration
            side_to_trade = row.get('side_to_trade', 'BOTH')
            
            if side_to_trade == 'YES':
                # Only trade token1 (YES side)
                deets = [deets[0]]
            elif side_to_trade == 'NO':
                # Only trade token2 (NO side)
                deets = [deets[1]]
            # else: BOTH - trade both sides (default behavior)
